...
def fix_tasks(
    pr_number: int, why: str, failed: Sequence[str] = (), *, gate: str | None = None
) -> list[TaskSpec]:
    """The seeded task graph for a fix round — deliberately one task.

    ``gate`` is the operator's ``sandbox.gate_command`` when one is set; it
    becomes the task's mechanical verify command now that no plan phase
    authors any. Host-authored, so the builder still never writes its own
    exam. With no gate known the task carries no verify commands and the
    PR's own CI checks — which the acceptance gate already polls — remain
    the mechanical arbiter.
    """
