...
#
#   R0  Secret command surface (`ls`, `rm --help`) — round 1's cleanup used
#       a rm syntax v0.35 rejects; the accepted form is verified here.
...
#
# This script runs in CI ONLY: dispatch the "sbx probe" workflow
# (.github/workflows/sbx-probe.yml), which installs real sbx on the Actions
# runner and runs this against it. Probe scripts are never to be run on
# customer or user devices.
#
#   ./scripts/spike-46-agent-session-probe.sh [report-file]
#
# Append-only report (default: spike-46-report.txt); the workflow publishes
# it in the run summary and as an artifact — attach it to issue #46.
#
# Safety properties:
...
# ---------------------------------------------------------------------------
log "R0: secret surface (issue #96: rm --env alone is rejected on v0.35)"
run "${SBX[@]}" secret ls
...
log "DONE — the workflow publishes $REPORT (run summary + artifact); attach it to issue #46"
