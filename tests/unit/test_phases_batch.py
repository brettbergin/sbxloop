...
class BatchStubAgent:
    """Answers shell.batch jobs with scripted per-command exits; captures
    every JobRequest."""
...
class TestVerifyBatching:
    def test_all_commands_ride_one_job(self) -> None:
        ...
        outcome = runner(agent).verify(record(["pytest -q", "ruff check .", "mypy ."]))
    ...
    def test_duplicate_commands_deduped(self) -> None:
        agent = BatchStubAgent()
        runner(agent).verify(record(["pytest -q", "pytest -q", "mypy ."]))
    ...
    def test_failures_reported_per_command(self) -> None:
        ...
        outcome = runner(agent).verify(record(["true", "exit 2"]))
    ...
    def test_no_commands_submits_no_job(self) -> None:
        agent = BatchStubAgent()
        outcome = runner(agent).verify(record())
        assert outcome.passed
