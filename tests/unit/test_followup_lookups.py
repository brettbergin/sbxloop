...
def test_issue_evidence_accepts_each_forges_issue_path(scheme):
    ...
    github_url = f"{scheme}://github.com/o/r/issues/12"
    assert issue_evidence(existing(html_url=github_url), "o/r").url == github_url
    for wrong in (
        f"{scheme}://gitlab.example.com/o/other/-/issues/12",
        f"{scheme}://github.com/o/r/pull/12",
        f"{scheme}://[broken/o/r/-/issues/12",
        "ftp://gitlab.example.com/o/r/-/issues/12",
    ):
