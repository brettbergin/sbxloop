...
- [Agent backends](#agent-backends-copilot-or-claude)
...
the schedule) or `cron = "0 7 * * mon-fri"` (read in `timezone`, the
daemon's `run_cap_timezone` when unset). Every tick queues the ask as a
...
daemon may run on schedules alone. `!sbx schedules` (or `sbxloop daemon ctl schedules`) lists them with the last fire and the next due; `schedules pause <name>` parks one — its ticks are skipped, not queued, until
`schedules resume <name>` — without pausing the daemon.
...
| `[[schedules]]`                                                                | none                                            | Workloads the daemon asks for by itself, on a cadence (#761): `name`, `profile` (a `[[workloads]]` entry), `ask`, and exactly one of `every` (`"24h"`, `"90m"`, `"1h30m"`; at least `1m`) or `cron` (five fields, names allowed: `0 7 * * mon-fri`), plus `timezone` (IANA; `[daemon] run_cap_timezone` when unset). A tick still live at the next due is skipped, not queued; `schedules pause <name>` parks one.                                                                                                                                                                                                          |
