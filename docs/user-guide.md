...
| `[[agents]]`                                                                                                               | none                                                                                                                                                                                                                | Agents beside the built-ins (Angie as `concierge`/`@angie`, planner, builder, critic, operator), or changes to the built-in with that `slug` (only keys set to a non-default value); an entry on a retired name such as `weather` makes a new agent. `slug` and `aliases` (@-names; an alias never names another agent), `name`, `description`, `instructions` (the persona), `avatar` (an emoji or 1-2 characters, never a URL), `color` (`#rrggbb`), `model` (unset: the role's model), `roles` (`lead`, `planner`, `builder`, `critic`, `operator`), `tools` / `skills` / `mcp` (unset: what the role gets; a list only narrows it; `mcp` names `[[mcp]]` entries; `memory` in `tools` gives the agent its `remember`/`recall`/`forget` tools in runs and chat), `credentials` (`[[credentials]]` names), `interests`, `can_start` (`code`, `workload`; needs a role), `max_runs_per_day`, `enabled`. No egress keys: egress stays with `[policy]` and `[[workloads]]`. An unknown credential, server or tool fails the load. |
