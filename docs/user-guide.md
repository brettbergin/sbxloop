...
| `[[agents]]`                                                                                       | none                                                                                                             | Agents beside the built-ins (Angie as `concierge`/`@angie`, planner, builder, critic, operator), or adjustments to the built-in with the same `slug` (only the keys an entry sets to a non-default value). `slug` and `aliases` (the @-names; an alias never names another agent), `name`, `description`, `instructions` (the persona), `avatar` (an emoji or 1-2 characters, never a URL), `color` (`#rrggbb`), `model` (unset: the role's model), `roles` (`lead`, `planner`, `builder`, `critic`, `operator`), `tools` / `skills` / `mcp` (unset: what the role gets; a list only narrows it; `mcp` names `[[mcp]]` entries), `credentials` (`[[credentials]]` names), `interests`, `can_start` (`code`, `workload`; needs a role), `max_runs_per_day`, `enabled`. No egress keys: hosts and allow patterns are refused, egress stays with `[policy]` and `[[workloads]]`. An unknown credential, server or tool fails the load. |
