...
| `[agent_team] max_chain_depth` / `max_agent_runs_per_day`                                          | `2` / `4`                                                                                                        | What agents may start on their own (`start_run`, `file_issue`, offered to an agent whose `[[agents]]` entry declares `can_start`). Work a person asked for is depth 0, so the first run an agent starts is depth 1; an agent working at `max_chain_depth` may start nothing, and `max_chain_depth = 0` stops agent-started work entirely. `max_agent_runs_per_day` is the daily cap for an agent whose spec names no `max_runs_per_day` of its own. The workspace pool (`[daemon] max_runs_per_day`, `daily_token_budget`) still applies on top.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
