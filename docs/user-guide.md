...
or `gitea`, whose backend answers every role too, with what Gitea lacks
said plainly: no merge queue (the landing merges directly), no review
threads (a finding is a review comment the loop answers with a
change-level comment quoting it, and never resolves), no bot flag
(`[vcs] bot_logins` names the automated reviewers; everyone else is a
person), and tokens that never expire (the doctor says so). `sbxloop doctor` prints one
...
branch's own access description. A Gitea token cannot read its own record,
so its row is judged endpoint by endpoint and says the token never
expires. A repository on another forge takes
...
either. On Gitea the staging is the same and the write is one
contents-API changeset; an executable bit cannot be set through that API
(it has no file mode), so a new executable lands `100644` and the log
says so, and a fix round commits the difference on top of the branch
(Gitea has no call that moves a branch, and deleting one closes its pull
request). Every fix round re-delivers onto the same
...
| `[vcs] kind`                                                                                       | `github`                                                                                        | The version-control backend the repositories live on (#1009): `github`, `gitlab` or `gitea`. `gitlab` and `gitea` answer every role (GitLab: a merge train only where the project has one, a delivery as one commits-API changeset; Gitea: no queue and no review threads, a delivery as one contents-API changeset with a fix round committed on top, see Delivery); `gitea` loads and `sbxloop doctor` reports it as not yet implemented. A `gitlab` or `gitea` configuration must also set `[vcs] api_url`. Anything else is refused at load, naming the three. A `[[github.repos]]` entry may set its own `kind`. Locked from the concierge by default.                                                                                                                                                                                            |
...
| `[vcs] bot_logins`                                                                                 | `[]`                                                                                            | The logins of automated reviewers on a forge that has no bot signal of its own (Gitea): a listed login's review is read as a bot's (one fix round, never a block, like a GitHub App's); every other reviewer is a person until listed, because treating a person as a bot drops their feedback after one answer. GitHub and GitLab read their own flag and ignore this key.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `[[github.repos]] bot_logins`                                                                      | unset                                                                                           | This one repository's automated reviewers; unset uses `[vcs] bot_logins`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
