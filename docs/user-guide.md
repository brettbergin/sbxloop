...
An operator's `[[vcs.repos]]` entry can override `cpus` and/or `memory`
...
under `[vcs.repos.openai]`; the rest keep the default.
...
(`[sandbox] workspace` or a `[[vcs.repos]]` entry), else the git checkout
...
catalogue, `vcs.repos` for an unconfigured repository, `workload.default`
...
message. A `[[vcs.repos]]` entry can set `env` for its own runs; a set
...
wins over the registry's. A `[[vcs.repos]]` entry may carry its own
...
ends the run at provisioning with the command in the error, and `keep_on_failure` keeps the sandbox for `sbxloop shell`. A `[[vcs.repos]]` entry may carry its own `apt_packages` or
...
services, is the verification. A `[[vcs.repos]]` entry sets the mode per
...
- a **daily token budget** (`daily_token_budget`, unset by default): the
  input and output tokens every run and every chat turn reported since
  00:00 in `run_cap_timezone`. Once the day's total reaches it, no new run
  starts and no chat turn is answered until the next day: a turn a person
  starts, in a channel or on a bridge, is refused before anything reaches
  the model and the refusal is the reply (the day's tokens against the
  budget and when it resets), exactly as an agent's follow-up turn already
...
- a **concurrency cap** (`max_concurrent_runs`, default 1): how many runs
  ...
  never work the same repository at once (the second waits in the queue),
  and a repository's checkout is not refreshed while a run is using it.
  `status` lists every run in flight; a bare `cancel` means the oldest;
- **warm sandbox sets** (`warm_pairs`, default 0): how many runs' worth of
  sandboxes the daemon keeps provisioned ahead of dispatch, microVMs booted
  and workers installed, under run ids no run has taken yet. A fresh run
  takes one and skips the boot and the install ladder (a minute or more per
  run in the field); a resume, a tool run and a run needing a different
  configuration provision as before. A set older than `warm_ttl_s` (default
  six hours), from another configuration, or missing a sandbox is retired.
  `status` reports `warm.ready` against `warm.target`, and `sbxloop sandbox prune` leaves warm sets alone;
...
- **reconciliation**: on start, and every tick (also while other runs
  execute), runs the store still shows in flight with no process behind them are
...
by default (`max_concurrent_runs`), counted across every configured
repository. Ship it as a systemd user service with
...
`[concierge] enabled | model | timeout_s | max_tool_calls | session_turns | max_concurrent_turns | github_tools | create_issues | edit_config | config_locked`
...
`daemon.restart`, `repository.resume`, `schedule.*`). `POST /v1/repositories`
registers a repository (admitted for work at once, polled from the next
start), `PATCH /v1/repositories/{id}` enables, disables or re-bases one and
`DELETE /v1/repositories/{id}` forgets a registration; see "Where a
repository is registered" below. Not offered remotely, by design:
...
what to offer: `local`, `oidc`, `policy_version: 1` and `oidc_session_max_age_s`,
and `assistant_name`, the name the assistant answers to (see `[[agents]]`).
The `oidc` object contains `id`, `label`, `authorize_url`, `client_id`, `scopes`,
`end_session_url` and `native_redirect_uris`, and is null when the section is
off or the provider's discovery document cannot be read. `auth.oidc` appears in
`GET /v1/capabilities` features only when the section is enabled.

Set `[api] local_auth_enabled = false` to require OIDC for people. This disables
both password login and registration, including using a human account's password
through the client-credentials grant. Existing human tokens without an OIDC
session are refused too, so people sign in again after the switch. Machine
clients created with `sbxloop api client create` keep working. The providers
route reports `local: false` even when discovery fails; an IdP outage never
enables passwords. The default remains `true` for existing installations.

On upgrade, existing OIDC-only accounts must sign in again because old tokens
have no durable session record. Existing local or linked accounts keep their
legacy tokens while local authentication is enabled; disabling it refuses those
tokens too, since their original sign-in method cannot be established.

An OIDC login has an absolute lifetime of `session_max_age_s` (eight hours by
default, at most one day); rotating refresh tokens cannot extend it. At that
boundary the browser must start OIDC again, which rechecks provider access and
group claims. Each access request and refresh checks the durable session and
the current local account, so new requests and refreshes observe local revocation
immediately. Existing streams close on their next periodic access check.

Configure the provider's **back-channel logout URI** to reach
`https://api.example.com/v1/auth/oidc/backchannel-logout` directly (or the same
route behind the browser client's API proxy). The route accepts a form-encoded
`logout_token`, signed by the configured provider; it requires no bearer token
and must not be redirected to an interactive login. It validates issuer,
client audience, signature, event, issue time, subject/session identifiers and
replay id before revoking matching application sessions. It accepts the original
OIDC logout format without `exp` used by Authentik, but always limits the token's
issue time to five minutes plus configured clock skew, and checks `exp` when
present. Repeated delivery cannot revoke sessions established after the first
delivery. Responses are not cacheable. In Authentik select back-channel logout
and set its Logout URI to this route.

Ending provider sessions can revoke app sessions through these signed events.
Changing a group alone may not cause the provider to emit a logout: end that
person's provider sessions for immediate removal, or the next OIDC login will
recheck their groups within the absolute lifetime. Verify the provider's event
delivery after configuring it. Deployment checks may read the public
`policy_version` and `oidc_session_max_age_s`; `auth.sso_policy` and, when enabled,
`auth.oidc.backchannel_logout` also appear in authenticated capabilities.
...
- **Native apps:** a phone or desktop app signs in through the system browser
  and returns on a private-use scheme redirect such as
  `com.example.app:/oauth2/callback`. Add that URI to the provider's strict
  redirect URIs beside the web one, and to `native_redirect_uris` below.
...
# native_redirect_uris = ["com.example.app:/oauth2/callback"]  # a native app
...
accepted for `localhost` only). A native app's redirect goes in
`native_redirect_uris` instead: a private-use scheme in reverse-DNS form
(RFC 8252 section 7.1), such as `com.example.app:/oauth2/callback`; a bare
scheme like `app:/cb` could be claimed by any app on the device and is refused
at load. A `redirect_uri` the client presents must be one of `redirect_uris` or
`native_redirect_uris` exactly, or the exchange is refused before the provider
is called. With a native redirect configured, the `oidc` object lists them in
`native_redirect_uris` and `auth.oidc.native` appears in capabilities features.
...
unlinked local account with the same address, but only when both sides vouch
for it: the ID token says `email_verified: true`, and the local account's
address was set by the installation's first registration, by an invite
addressed to it, or by an earlier verified provider claim. An address a
person typed in themselves, at an open invite or through
`PATCH /v1/users/me`, is never linked, so a member cannot capture a
colleague's first sign-in by taking their address first; an account from
before this rule counts as typed in, so its first sign-in creates a second
account to merge (below). The linked account keeps `auth_source = "local"`
and its password keeps working while `local_auth_enabled` is true, the link is recorded as `auth.oidc.linked`,
and the sign-in that links leaves the account's role alone (the next one
follows `owner_groups` and `admin_groups` as usual). An unverified email, one
nobody vouches for locally, or one whose account is already linked to
another identity, still gets a new account. An email the provider has not
verified is never stored either: the new account gets the undeliverable
address, and a later sign-in keeps the address on record. Linking trusts the
provider's `email_verified` claim completely: whoever the provider admits
with a verified address takes over the account holding it, including the
...
With `[push] enabled = true` and `relay_url` set, a signed-in person can
...
key and nothing else. On registration the daemon hands it the device's
push token once and keeps only the opaque handle it returns (the token
itself is stored as a digest and its last six characters). A ping carries a
kind (`mention`, `gate`, `work`, `failure`, `test`), a random notification
reference, the channel id and the client's own name for this server; the
relay turns that into a generic alert. **Content never leaves the server:**
the device fetches the title and text from
`/v1/users/me/notifications/{ref}` with the person's own token and shows
that instead. Neither the token nor the handle appears in an event or a log
line; a device is logged by its id.

Pushes are live: the daemon starts reading the chronology from its head
each time it starts, so history, a restart and whatever happened while it
was down never produce a burst of stale alerts. A person is never notified
of their own message, and each device's preferences (per kind, and per
channel `all`, `mentions` or `none`) narrow what reaches it. A device the
relay reports as gone is removed; one whose handle the relay no longer
recognises (its secret was rotated) is enrolled again the next time the app
registers it. The rendered notifications are kept as long as the
chronology (`[api] replay_retention_s`).
...
or persistently in `sbxloop.toml`, under the forge section, whatever forge
...
[[vcs.repos]]
repo = "you/your-repo"   # a repo sbxloop may act on
...
The legacy spelling — `[[github.repos]]` entries, or a single
`[github] repo = "you/your-repo"` with the delivery settings beside it —
still loads, folded into the same list with one notice in the daemon log
(`config.repos_legacy`); a file that declares repositories under both, and
not the same ones, is refused by name. `[github]` keeps the GitHub backend's
own settings: where GitHub is, how the loop names what it writes, the
identity and reviewers it acts with.

Several repositories are registered as several entries — each carries its
own delivery settings, an `enabled` switch and an optional per-repo token
...
run it again after renaming a label. A remote operator does not need the
host for it: every entry of `GET /v1/repositories` carries a `labels` state
(`compliant`, `incomplete` with the missing names, or `unknown` when nobody
has been able to look), read back by the daemon on
`[daemon] label_check_interval_s`, and
`POST /v1/repositories/{id}/labels/sync` creates the missing ones through
the daemon's own forge sandbox — a repository that already carries them is
read and left alone. `sbxloop doctor` reports missing labels and a
...
`[sandbox] workspace` into the matching `[[vcs.repos]]` entry** as
...
**Where a repository is registered** is the daemon's database, the way
schedules are: the daemon imports the file's `[[vcs.repos]]` entries into it at
first sight (the log and the control channel say which, once) and from then on
runs by the registration. `POST /v1/repositories` registers one (`repository`,
`forge`, `enabled`, `deliver_base`), `PATCH /v1/repositories/{id}` enables,
disables or re-bases one and `DELETE /v1/repositories/{id}` forgets one; each
is a recorded operation, and a client such as Angie offers them as a screen.
A registration is admitted for work at once; what the daemon *polls* was built
at start, so a change to the enabled set says a restart is needed. The file's
entry keeps everything else — labels, templates, `workspace`, `env`,
packages, model overrides — folded under the registration of the same name; a
new entry in the file is registered at the next start, and its `enabled` and
`deliver_base` are the initial values only: after the import the registration
wins, and `sbxloop doctor` names an entry the file still spells differently. A
registration removed through the API is not imported again from the file.
...
`{repo}` as placeholders; each can be overridden per `[[vcs.repos]]`
...
set `[github] bot_login` (per repository in `[[vcs.repos]]`) to the
...
boots. A `[[vcs.repos]] token_env` stays an explicit per-repo PAT
...
| `[vcs.repos.agent_models] <phase>`                                                                                         | unset                                                                                                                                                                                                               | Sparse per-repository overrides for any of the eight role keys. Unset roles inherit global `[agent.models]`, then `model`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `[[vcs.repos]] cpus / memory`                                                                                              | `inherit [sandbox]`                                                                                                                                                                                                 | Sparse overrides for the run agent only; helpers and concierge retain their own allocations. See Sandbox CPU and memory for cutover and recreation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `[[agents]]`                                                                                                               | none                                                                                                                                                                                                                | Agents beside the built-ins (Angie as `concierge`/`@angie`, planner, builder, critic, operator), or changes to the built-in with that `slug` (only keys set to a non-default value); an entry on a retired name such as `weather` makes a new agent. An entry for `concierge` renames the assistant: its `name` is what it calls itself in chat, what the agent directory, the preference prompts and refusals show and what `GET /v1/auth/providers` reports as `assistant_name` before sign-in, and its `aliases` replace `@angie` (list `angie` to keep it); its slug, chat sessions and stored history are unchanged. `slug` and `aliases` (@-names; an alias never names another agent), `name`, `description`, `instructions` (the persona), `avatar` (an emoji or 1-2 characters, never a URL), `color` (`#rrggbb`), `model` (unset: the role's model), `roles` (`lead`, `planner`, `builder`, `critic`, `operator`), `tools` / `skills` / `mcp` (unset: what the role gets; a list only narrows it; `mcp` names `[[mcp]]` entries; `memory` in `tools` gives the agent its `remember`/`recall`/`forget` tools in runs and chat), `credentials` (`[[credentials]]` names), `interests`, `can_start` (`code`, `workload`; needs a role), `max_runs_per_day`, `enabled`. No egress keys: egress stays with `[policy]` and `[[workloads]]`. An unknown credential, server or tool fails the load. |
| `[memory] enabled` / `max_items_per_agent` / `max_item_chars` / `prompt_budget_chars`                                      | `true` / `500` / `1000` / `4000`                                                                                                                                                                                    | Each agent's long-term memory, managed through `/v1/agents/{slug}/memories`. A memory learned in a channel is shown only there (a global one everywhere); past `max_items_per_agent` the agent's oldest unpinned memory is dropped; text is cut to `max_item_chars`; a memory block adds at most `prompt_budget_chars` to a prompt. `enabled = false` refuses new memories and keeps stored ones out of recall; they stay listable and deletable.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `[agent_team] max_chain_depth` / `max_agent_runs_per_day`                                                                  | `2` / `4`                                                                                                                                                                                                           | What agents may start on their own (`start_run`, `file_issue`, offered to an agent whose `[[agents]]` entry declares `can_start`). Work a person asked for is depth 0, so the first run an agent starts is depth 1 and a handoff peer's is depth 2; an agent at `max_chain_depth` may start nothing, and `max_chain_depth = 0` stops agent-started work entirely. `max_agent_runs_per_day` is a ceiling an agent's own `max_runs_per_day` lowers, never raises. Both bounds count every start when it is made, an issue filed without queueing included, so a queued issue counts before the poll that discovers it. The workspace pool (`[daemon] max_runs_per_day`, `daily_token_budget`) still applies on top of a run, but not to an unqueued issue, which runs nothing. An agent offered these tools is not also offered the concierge's `create_issue`, `label_issue_for_run`, `start_workload`, `start_entrygraph` or `create_schedule`, nor a peer it hands off to, so its starts always pass these bounds.                                                                                                                                                                                                                                                                                                                                                                                   |
| `[agent_team] chronicle` / `max_posts_per_run` / `progress_interval_s`                                                     | `normal` / `12` / `120.0`                                                                                                                                                                                           | What a run says in the channel that asked for it, posted under the name of the agent doing the work: the plan by the planner, each finished task by its agent, the verdict by the critic, a steering reply by the agent that was asked, and the delivery or a notice by the lead. `quiet` keeps only what ends a run (its delivery, and a notice when it stopped); `off` posts nothing. Progress is coalesced to at most one post every `progress_interval_s` seconds, and a run makes at most `max_posts_per_run` posts - the delivery and a terminal notice are posted above that cap, and a silenced channel hears them too. A run no channel asked for posts nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `[collaboration] max_chain_depth` / `window_s` / `channel_turns_per_window` / `agent_turns_per_window` / `pair_cooldown_s` | `4` / `600` / `20` / `6` / `60`                                                                                                                                                                                     | What bounds agents addressing each other in a channel. Naming an agent in an agent's own reply queues a follow-up turn for it; a chain of those may go `max_chain_depth` deep after one human turn, a channel takes at most `channel_turns_per_window` agent turns and one agent at most `agent_turns_per_window` of them within `window_s` seconds, and one agent waits `pair_cooldown_s` before addressing the same agent again. A refused follow-up is recorded as `collaboration.followup.suppressed` with its reason. The channel's silence and `[daemon] daily_token_budget` apply as well.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `[collaboration] ambient` / `ambient_model` / `ambient_window_messages` / `ambient_max_per_hour`                           | `false` / unset / `5` / `6`                                                                                                                                                                                         | Whether a channel participant in `ambient` mode may speak without being addressed, and what it costs to decide. With `ambient = true`, a new message is matched against each ambient agent's `interests` (the new message alone, not the messages before it); an agent none of whose interests match is not considered further and no model is called. One that does match passes the guardrails above, then a short relevance call on `ambient_model` (the concierge's model when unset), which reads the last `ambient_window_messages`, answers RELEVANT or PASS; the call runs after the agents the person addressed have answered, so it never delays them; a PASS posts nothing and records `collaboration.followup.suppressed` with reason `ambient_pass`. One agent speaks at most `ambient_max_per_hour` times in a channel. `ambient = false` turns all of it off, prefilter and classifier alike.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `[concierge] config_locked`                                                                                                | `["policy", "mcp", "credentials", "registries", "vcs.kind", "vcs.api_url", "vcs.token_env", "vcs.repos.kind", "vcs.repos.token_env", "agents", "github.repos.kind", "github.repos.token_env", "telemetry.dsn_env"]` | Dotted prefixes chat may read but not change — egress, tool grants, the forge and credential names by default, because widening those from a chat mention is a different weight of act than raising a cap. A repository's own delivery settings under `[[vcs.repos]]` are not locked. A prefix covers everything under it; `*` matches one segment; indices are not segments. Remove one here, on the host, to allow it from chat.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `[concierge] max_concurrent_turns`                                                                                         | `1`                                                                                                                                                                                                                 | How many chat turns may run at once (1 to 16), each on its own worker in the concierge sandbox. Turns in one channel still run one after another; different channels run side by side up to this many. Each turn keeps its own speaker, session and interrupted-call recovery, so turns in different channels cannot read each other's; host tools still run one at a time. Turns that resume the same concierge session run one at a time in the order they arrived whatever this is set to, so the chat bridge turns (Discord, Slack, Mattermost, the console), which all resume the one default session, never interleave.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `[[vcs.repos]] repo`                                                                                                       | none                                                                                                                                                                                                                | A repository sbxloop works with, one entry each, whatever forge it lives on (#2255); with none declared the forge is off and a run stops after its gate. Each entry carries its own `enabled`, `kind`, `workspace`, delivery settings, labels, token variable and sandbox overrides; the `[daemon]` guardrails stay daemon-wide. `[[github.repos]]` is the legacy spelling and still loads as this list. Registration (which repositories, `enabled`, `kind`, `deliver_base`) is imported into the daemon's database at first sight and changed there through `POST`/`PATCH`/`DELETE /v1/repositories`; an entry here keeps the repository's other settings, and a new entry is registered at the next start.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `[github] repo`                                                                                                            | unset                                                                                                                                                                                                               | Legacy: one repository, read as a single `[[vcs.repos]]` entry carrying the `deliver_base`, `create_repo` and `create_public` beside it. The rest of `[github]` — `pr_title_template`, `commit_message_template`, `branch_prefix`, `bot_login`, `reviewers`, `api_url` — is the GitHub backend's own and applies to every GitHub repository unless an entry overrides it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `[vcs] kind`                                                                                                               | `github`                                                                                                                                                                                                            | The version-control backend the repositories live on (#1009): `github`, `gitlab` or `gitea`. `gitlab` answers every role (a merge train only where the project has one; a delivery is one commits-API changeset, and a fix round rewrites the branch rather than deleting it, see Delivery); `gitea` loads and `sbxloop doctor` reports it as not yet implemented. A `gitlab` or `gitea` configuration must also set `[vcs] api_url`. Anything else is refused at load, naming the three. A `[[vcs.repos]]` entry may set its own `kind`. Locked from the concierge by default.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `[vcs] api_url`                                                                                                            | unset                                                                                                                                                                                                               | The forge's API root. Accepts HTTP or HTTPS for `gitlab` and `gitea`; HTTP sends the forge token unencrypted. For `github` it requires HTTPS and is the same setting as `[github] api_url`: a `[vcs]` value fills it, and two different values are refused at load.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `[vcs] token_env`                                                                                                          | per kind                                                                                                                                                                                                            | The host variable the forge token is read from, by name; the value lives in `secrets.env`. Unset: `GITLAB_TOKEN` for `gitlab`, `GITEA_TOKEN` for `gitea`. For `github` the credential is `GH_TOKEN`/`GITHUB_TOKEN` or the App installation, and a `[[vcs.repos]] token_env` still wins. The doctor's repository row says which variable and whether it is set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `[[vcs.repos]] kind`                                                                                                       | unset                                                                                                                                                                                                               | The forge this one repository lives on; unset uses `[vcs] kind`. A run's own github box follows it; the daemon's shared polling box follows `[vcs] kind`, so every repository one daemon polls must share the daemon's kind today.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `[landing]`                                                                                                                | see above                                                                                                                                                                                                           | `deliver_draft`, `max_review_rounds`, `max_ci_rounds`, `retry_rounds`, `followups`, `followup_label`, `max_followups_per_run`, `ci_poll_interval_s`, `ci_poll_min_s`, `ci_settle_s`, `ci_timeout_s`, `merge_method`, `delete_branch_on_merge`, `merge_update_attempts`, `required_checks`, `ignore_checks`, `ignore_reviewers`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `[daemon] trigger_label` … `workload_label`                                                                                | `sbxloop:run` …                                                                                                                                                                                                     | The issue labels: `trigger_label`, `in_progress_label`, `completed_label`, `failed_label`, `blocked_label`, `gated_label`, `workload_label` (queues the issue as a workload); each can be renamed per `[[vcs.repos]]` entry. `sbxloop init-repo` creates them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `[daemon] label_check_interval_s`                                                                                          | `3600.0`                                                                                                                                                                                                            | How often a registered repository's labels are read back, so the API's repositories list can say whether it carries the set the loop applies (`compliant` / `incomplete` / `unknown`). One listing call per repository per interval, one repository per tick, enabled repositories only, and never a sandbox boot of its own (it rides the one polling made); `0` turns the reading off. `POST /v1/repositories/{id}/labels/sync` creates the missing ones on demand, as `sbxloop init-repo` does from the host.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `[daemon] max_concurrent_runs`                                                                                             | `1`                                                                                                                                                                                                                 | Runs executing at once, 1 to 4. At 1 a tick dispatches a run and settles it before the next; above 1 a tick starts runs up to this many and returns, and the next tick settles what finished. Two code runs never work the same repository at once, and a repository's checkout is not refreshed while a run is using it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `[api.oidc] native_redirect_uris`                                                                                          | `[]`                                                                                                                                                                                                                | Redirect URIs a native app may present beside `redirect_uris`, matched exactly: each a private-use scheme in reverse-DNS form (RFC 8252 section 7.1), such as `com.example.app:/oauth2/callback`. A bare scheme (`app:/cb`), `http` or `https`, a fragment or a credential is refused at load. Set, `GET /v1/auth/providers` lists them on the `oidc` object and `GET /v1/capabilities` gains `auth.oidc.native`. Register the same URI with the provider.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `[api.oidc] session_max_age_s`                                                                                             | `28800`                                                                                                                                                                                                             | Absolute OIDC application session lifetime, from 300 to 86400 seconds. Access and refresh tokens cannot outlive it; provider back-channel logout can revoke sessions sooner.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `[api.oidc] link_verified_email`                                                                                           | `false`                                                                                                                                                                                                             | Link a first sign-in to the unlinked local account whose email matches, when the provider says the email is verified and the local address came from the first registration, an invite addressed to it or an earlier verified claim (never from the person editing their profile); the linking sign-in leaves the role alone. Turn it on only if people cannot change their email at the provider and the provider asserts `email_verified` only for addresses it has checked; otherwise anyone the provider admits could take over the account with that address, the owner's included. Off, or unverified on either side, the person gets a new account of their own.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `[push] enabled` / `relay_url`                                                                                             | `false` / `""`                                                                                                                                                                                                      | Push notifications to people's devices through a push relay (see [Push notifications](#push-notifications)). Registration and the `push.apns_relay` feature need both: on, and an `http(s)://` relay URL with a host and no credentials, query or fragment (refused at load otherwise). Workspace-wide: there is no per-repo override. Never changed from chat.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `[push] timeout_s` / `max_attempts` / `backoff_s` / `backoff_max_s`                                                        | `10.0` / `5` / `2.0` / `300.0`                                                                                                                                                                                      | Each call to the relay waits at most `timeout_s`. A push the relay could not deliver for now (a retryable upstream failure, a rate limit, no answer) is tried up to `max_attempts` times, waiting `backoff_s` then twice as long each time, at least the relay's `Retry-After` and never more than `backoff_max_s`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `[push] max_devices_per_user`                                                                                              | `20`                                                                                                                                                                                                                | How many devices one person may register; one more is `409 device_limit_reached`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
