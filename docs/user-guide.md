...
read paths (the repository, issues, checks and the base's rules) and stops a
run, naming the operation, at its first write until the review and landing
paths land; or `gitea`, which loads ahead of its backend and which
...
| `[vcs] kind`                                                                                       | `github`                                                                                        | The version-control backend the repositories live on (#1009): `github`, `gitlab` or `gitea`. `gitlab` answers the read paths and fails a run closed, naming the operation, at its first write until its review and landing paths land (#1018, #1019); `gitea` loads and `sbxloop doctor` reports it as not yet implemented. A `gitlab` or `gitea` configuration must also set `[vcs] api_url`. Anything else is refused at load, naming the three. A `[[github.repos]]` entry may set its own `kind`. Locked from the concierge by default.                                                                                                                                                                                                                                                                                                            |
