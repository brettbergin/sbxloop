...
forge a repository lives on: `github`; `gitlab`, whose backend answers the
read paths (the repository, issues, checks and the base's rules), the merge
request, its review threads and its landing (the un-draft, the rebase, the
merge, and the merge train where the project has one), and stops a run,
naming the operation, at the remote commit until it lands; or `gitea`, which loads ahead of its backend and which
...
| `[vcs] kind`                                                                                       | `github`                                                                                        | The version-control backend the repositories live on (#1009): `github`, `gitlab` or `gitea`. `gitlab` answers the read paths, the merge request, its review threads and the landing (a merge train only where the project has one), and fails a run closed, naming the operation, at the remote commit (#1020) until it lands; `gitea` loads and `sbxloop doctor` reports it as not yet implemented. A `gitlab` or `gitea` configuration must also set `[vcs] api_url`. Anything else is refused at load, naming the three. A `[[github.repos]]` entry may set its own `kind`. Locked from the concierge by default.                                                                                                                                                                                                                                   |
